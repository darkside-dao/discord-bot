module.exports = {
  ...
  async execute(ctx) {
    ...
    if (!mod && !cmd && !scmd) {
      embeds = [
        {
          title: "Eee! I'm Darkside Guard!",
          description:
            "I help people with management to Discord groups! Here are some of my features:",
          fields: [
            {
              name: "Reaction-based interaction",
              value:
                "Many of my functions are based on reacts! This keeps typing to a minimum",
            },
            {
              name: "Several question types",
              value:
                "Guard currently supports several question types, including:" +
                "\n- Multiple choice" +
                "\n- Checkbox" +
                "\n- Freeform" +
                "\n- Attachment" +
            ...
            },
            {
              ...
              value:
                "[https://discord.gg/KKAMqpG7r5](https://discord.gg/KKAMqpG7r5)",
              inline: true,
            },
            {
              name: "Follow my creators!",
              value:
                "[Twitter](https://twitter.com/darkside_dao) | " +
                "[Github](https://github.com/darkside_dao)",
              inline: true,
            },
          ],
      ...
      var mods = ctx.client.slashCommands
        .map((m) => m)
        .filter((m) => m.options);
      ...
      for (var m of mods) {
        ...
        var tmp = await ctx.client.utils.genEmbeds(
          ctx.client,
          cmds,
          (c) => {
      ...
      if (ug?.[0]) {
        ...
        for (var c of ug)
          e.fields.push({
            ...
            value: c.description ?? c.data.description,
          });
        embeds.push(e);
      }
    } else {
      ...
      if (cm.options)
        embeds[embeds.length - 1].fields.push({
          ...
          value: cm.options
            .map(
              (o) =>
                `**/${name.trim()} ${o.data.name}** - \`${o.data.description}\``
            )
            .join("\n"),
        });
      if (cm.extra)
