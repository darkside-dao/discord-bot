...
opts.push({
  ...
  usage: ["- View your own profile", "[user] - View another user's profile"],
  async execute(ctx) {
    var { client: bot } = ctx;
      ...
      embeds: [
        {
          ...
          footer: {
            text: `Level-up messages ${
              profile.disabled ? "are" : "are not"
...
opts.push({
  ...
  usage: ["[title] - Set your profile title"],
  async execute(ctx) {
    var {
      client: { stores },
    } = ctx;
    ...
    if (title.length > 100)
      return "Title too long; must be 100 characters or less";
...
opts.push({
  ...
  usage: ["[description] - Set your profile description"],
  async execute(ctx) {
    var {
      client: { stores },
    } = ctx;
    ...
    if (desc.length > 250)
      return "Description too long; must be 250 characters or less";
...
opts.push({
  ...
  usage: ["- Completely delete your profile"],
  async execute(ctx) {
    var {
      client: { stores },
    } = ctx;
