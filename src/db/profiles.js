...
class Profile {
  ...
  constructor(store, data) {
    this.store = store;
    for (var k of KEYS)
      if (data[k] !== null && data[k] !== undefined) this[k] = data[k];
  }
...
class ProfileStore {
  ...
  async getID(id) {
    try {
      var data = await this.db.query(`SELECT * FROM profiles WHERE id = $1`, [
        id,
      ]);
    } catch (e) {
      return Promise.reject(e.message);
  ...
  async update(id, data = {}) {
    try {
      await this.db.query(
        `
				UPDATE profiles SET ${Object.keys(data)
          .map((k, i) => k + "=$" + (i + 2))
          .join(",")}
				WHERE id=$1`,
  ...
  async handleExperience(user) {
    return new Promise(async (res, rej) => {
      ...
      if (profile.exp + amount >= nextLevel) {
        ...
        profile.level += 1;
        if (!profile.disabled)
          data.message = `Congrats $USER, you're now level ${profile.level}!`;
