...
module.exports = {
    ...
    return new Promise(async (res) => {
        ...
        embed: {
          ...
          description:
            typeof info.description == "function"
              ? info.description(arr[0], 0)
              : info.description,
          color:
            typeof info.color == "function"
              ? info.color(arr[0], 0)
              : info.color,
          footer: info.footer,
          fields: [],
      ...
      if (embeds.length > 1) {
        for (let i = 0; i < embeds.length; i++)
          embeds[i].embed.title +=
            extras.addition != null
              ? eval("`" + extras.addition + "`")
              : ` (page ${i + 1}/${embeds.length}, ${arr.length} total)`;
      }
      res(embeds);
    });
  },
  paginateEmbeds: async function (bot, m, reaction) {
    switch (reaction.emoji.name) {
      case "⬅️":
        ...
        await m.edit({
          embeds: [this.data[this.index].embed ?? this.data[this.index]],
        });
  ...
  getConfirmation: async (bot, msg, user) => {
    return new Promise((res) => {
      function msgListener(message) {
        ...
        if (STRINGS[0].includes(message.content.toLowerCase()))
      ...
      function reactListener(react, ruser) {
        if (react.message.channel.id != msg.channel.id || ruser.id != user.id)
          return;
      ...
      function intListener(intr) {
        ...
        if (intr.channelId !== msg.channel.id || intr.user.id !== user.id)
          return;
        ...
        if (BUTTONS[0].includes(intr.customId))
          return res({ confirmed: true, interaction: intr });
        else
          return res({
            confirmed: false,
            interaction: intr,
  ...
  awaitSelection: async (
    ctx,
    choices,
    msg,
    options = { min_values: 1, max_values: 1, placeholder: "- - -" }
  ) => {
    var components = [
      {
        type: 3,
    ...
    try {
      var resp = await reply.awaitMessageComponent({
        filter: (intr) =>
          intr.user.id == ctx.user.id && intr.customId == "selector",
        time: 60000,
      });
    } catch (e) {}
      ...
      components: [
        {
          ...
          components: components.map((c) => ({
            ...
            options: choices.map((ch) => ({
              ...ch,
              default: resp.values.includes(ch.value),
            })),
          })),
        },
      ],
  ...
  async awaitModal(ctx, data, user, ephemeral = false, time = 30000) {
    return new Promise(async (res) => {
      ...
      async function modListener(m) {
        ...
        if (!(m.customId == data.custom_id && m.user.id == user.id)) return;
